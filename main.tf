module "alb" {
  ...
  security_groups                  = concat([module.security_group_alb[each.key].security_group_id], try(each.value.security_groups_ids, [])) #[module.security_group_alb[each.key].security_group_id]
